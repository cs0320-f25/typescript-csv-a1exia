...
test("parseCSV skips extra blank lines", () => __awaiter(void 0, void 0, void 0, function* () {
    const results = yield (0, basic_parser_1.parseCSV)(EXTRA_ROW_CSV_PATH);
    expect(results).toContainEqual([]);
}));
//Pact C Tests
test("returns string[][] when no schema is provided", () => __awaiter(void 0, void 0, void 0, function* () {
    const results = yield (0, basic_parser_1.parseCSV)(STUDENTS_CSV_PATH);
    expect(Array.isArray(results)).toBe(true);
    expect(results[0]).toEqual(["Annie", "13", "annie@gmail.com"]);
}));
test("validates rows with schema (success cases)", () => __awaiter(void 0, void 0, void 0, function* () {
    const schema = zod_1.z.tuple([zod_1.z.string(), zod_1.z.coerce.number(), zod_1.z.string().email()]);
    const results = yield (0, basic_parser_1.parseCSV)(STUDENTS_CSV_PATH, schema);
    if ("success" in results[0]) {
        expect(results[0].success).toBe(true);
        expect(results[0].data).toEqual(["Annie", 13, "annie@gmail.com"]);
    }
}));
test("validates rows with schema (failure cases)", () => __awaiter(void 0, void 0, void 0, function* () {
    const schema = zod_1.z.tuple([zod_1.z.string(), zod_1.z.coerce.number(), zod_1.z.string().email()]);
    const results = yield (0, basic_parser_1.parseCSV)(STUDENTS_CSV_PATH, schema);
